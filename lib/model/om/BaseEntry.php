...
abstract class BaseEntry extends BaseObject  implements Persistent {
	...
	public function setText($v)
	{
		...
		if ($this->text !== $v) {
			$this->text = $v;
			$this->modifiedColumns[] = EntryPeer::TEXT;
		}
	...
	public function setSlug($v)
	{
		...
		if ($this->slug !== $v) {
			$this->slug = $v;
			$this->modifiedColumns[] = EntryPeer::SLUG;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 6; // 6 = EntryPeer::NUM_COLUMNS - EntryPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseEntry:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseEntry:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseEntry:save:pre') as $callable)
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(EntryPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseEntry:save:post') as $callable)
				{
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EntryPeer::TEXT)) $criteria->add(EntryPeer::TEXT, $this->text);
		if ($this->isColumnModified(EntryPeer::SLUG)) $criteria->add(EntryPeer::SLUG, $this->slug);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseEntry:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseEntry::%s', $method));
	  }
